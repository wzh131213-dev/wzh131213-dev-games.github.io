...
let selectedColor = 0xcb2d2d; // 重卡红
...
function triggerCheat() {
    ...
    if (inputPass === "wzh") {
        ...
        alert("⚡ 作弊码激活成功！《黑街 DJ》电音加速中！速度已提升至 100,000 km/min！");
...
function init3D() {
    ...
    // 自动在顶部按钮组注入切换视角按钮
    const topBtnGroup = document.getElementById('top-btn-group');
    if (topBtnGroup && !document.getElementById('btn-switch-view')) {
        const viewBtn = document.createElement('button');
        viewBtn.id = 'btn-switch-view';
        viewBtn.innerText = '📷 第一视角';
        viewBtn.style.cssText = 'background: #3b82f6; color: white; border: none; padding: 6px 12px; border-radius: 6px; font-weight: bold; cursor: pointer; margin-left: 8px;';
        viewBtn.onclick = toggleCameraMode;
        topBtnGroup.appendChild(viewBtn);
    }
...
function animate() {
    ...
    if (gameStarted && !isGameOver && !isVictory) {
        ...
        if (is2DMode) {
            // 2D 街机模式左右方向修正
            if (keys.left) arcadePlayer.x = Math.min(window.innerWidth - 120, arcadePlayer.x + 7);
            if (keys.right) arcadePlayer.x = Math.max(50, arcadePlayer.x - 7);
            ...
            // 相机位置控制（支持第一视角与第三视角切换）
            if (!isTransitioning) {
                if (cameraMode === 'first') {
                    // 第一视角：放置在驾驶舱内部，向前看
                    camera.position.set(playerX, 2.2 + playerY, totalDistance + 2.0);
                    camera.lookAt(playerX, 2.0 + playerY, totalDistance + 25);
                } else {
                    // 第三视角：后方跟随
                    const isLandscape = window.innerWidth > window.innerHeight;
