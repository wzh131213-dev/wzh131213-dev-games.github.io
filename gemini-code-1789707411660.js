...
function ensureViewButton() {
    ...
    if (!viewBtn) {
        ...
        viewBtn.style.cssText = 'position: fixed; top: 15px; right: 15px; z-index: 99999; background: #3b82f6; color: white; border: none; padding: 10px 16px; border-radius: 8px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.3); font-size: 14px;';
...
function animate() {
    ...
    if (gameStarted && !isGameOver && !isVictory) {
        ...
        if (is2DMode) {
            ...
            // 3D 模式：恢复正常直觉左右键（按左键向左减小X，按右键向右增大X）
